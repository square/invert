...
internal abstract class InvertCollectDependenciesTask : DefaultTask() {
  ...
  companion object {
     ...
     */
    fun resolveMonitoredConfigurationsMap(
  ...
  internal fun execute() {
    val computeCollectedFlattenedDependencyData = computeCollectedDependenciesForProject(
      ...
      transitiveDeps = monitoredConfigurationsMap.get()
    ...
    val collectedConfigurations = CollectedConfigurationsForProject(
      ...
      analyzedConfigurationNames = monitoredConfigurationsMap.get().keys.toSortedSet()
    )
  ...
  fun setParams(
    ...
    this.allConfigurationNames.set(allConfigurationNames)
    ...
    directDependencies.set(
      project.configurations
        .toSet()
        .filter {
          filteredConfigurationNames.contains(it.name) || annotationProcessors.contains(it.name)
        }
